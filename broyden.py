...
def matr(matrix): # строит матрицу
    return np.array([[matrix[0]], [matrix[1]]])

def broyden(startX, values, e):
    ...
    plot_graph(x1scale, x2scale, scale, x1, x2, 'Метод Бройдена')
    print_table(steps, x1, x2, f, "Метод Бройдена:")

broyden(startX, values, e)