...
def gradients(startX, values, e):
    ...
    while np.linalg.norm(dfX) > e:
        ...
        i += 1
        steps.append(i)
        print(np.linalg.norm(dfX))

    plot_graph(x1scale, x2scale, scale, x1, x2, 'Метод сопряжённых градиентов')
    print_table(steps, x1, x2, f, "Метод сопряжённых градиентов:")

gradients(startX, values, e)